    ...
    def main(self): 
        ...
        if os.name == "nt": 
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Nitro Generator || Made by zolo")  
        else:  
            print(f'Nitro Gen || zolo',
                  end='', flush=True) 
...
╚══════╝ ╚═════╝ ╚══════╝ ╚═════╝     ╚═╝  ╚═══╝╚═╝   ╚═╝   ╚═╝  ╚═╝ ╚═════╝      ╚═════╝ ╚══════╝╚═╝  ╚═══╝
        ...
        print(VER)
        time.sleep(2)  
        self.slowType(f"{Fore.MAGENTA}Made by Zolo{Fore.RESET}", .02)
        time.sleep(1) 
        self.slowType(
        ...
        for s in c: 
            ...
            if os.name == "nt": 
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Zolo") 
                print("")
            else: 
                print(
                    f'\33]0;Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Zolo\a', end='', flush=True)
        if len(valid) < 1:
            valid = "None"
        else:
            ', '.join(valid)
